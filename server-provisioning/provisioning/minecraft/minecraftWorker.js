...
function runContainer(docker, serverTemplate, image) {
    ...
    const dockerStartOptions = {
        ...
        Env: [`MOTD=${serverTemplate.name}\nHosted by MinecraftCloud`, 'EULA=true', 'INIT_MEMORY=512M', `MAX_MEMORY=${memory}M`, `TYPE=${type.toUpperCase()}`, `VERSION=${version}`, 'TZ=Europe/Berlin',  'JVM_XX_OPTS=-XX:+UseCompressedOops', `ICON=${mcConfig.defaultIcon}`],
        HostConfig: {
            PortBindings: {
                "25565/tcp": [
                    {

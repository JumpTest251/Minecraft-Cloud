...
CMD envsubst '\$PORT' < /etc/nginx/conf.d/default.conf.template > /etc/nginx/nginx.conf && nginx -g 'pid /tmp/nginx.pid; daemon off;' & npm run start --prefix user & npm run start --prefix server & npm run start --prefix analytics & wait -n
